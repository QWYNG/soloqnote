...
class Summoner():
    ...
    def get_match_data(self, summoner_name_data):
        ...
        my_ranked_stats = watcher.league.positions_by_summoner(
            my_region, me['id'])
        recentmatchlists = watcher.match.matchlist_by_account(
            my_region, me['accountId'])
        matches = recentmatchlists['matches']
        x = 0
        match_detail = []
        for x in range(20):
            match_detail.append(watcher.match.by_id(
                my_region, matches[x]['gameId']))
            x += 1
        ...
        for my_ranked_stat in my_ranked_stats:
            dict = {'my_ranked_stat': my_ranked_stat, 'me': me, "recentmatchlists": recentmatchlists,
                    'match_detail': match_detail, 'range': range(20), 'participantId': participantId, 'result': result}
        return (dict)
